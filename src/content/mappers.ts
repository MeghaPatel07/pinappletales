/**
 * Raw Firestore documents → typed content.
 *
 * Documents are written by an admin UI that evolves over time, so every field
 * is read defensively: a post saved before a field existed must still render
...
export function toEventForm(doc: RawDocument): EventForm {
  return {
    // The document id is the event id — see firestore.rules.
