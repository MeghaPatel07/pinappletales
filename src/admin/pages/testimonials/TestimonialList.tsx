...
export function TestimonialList() {
  ...
  const columns = useMemo<Column<Testimonial>[]>(() => [
    { key: 'testimonial', header: 'Testimonial', value: (item) => item.testimonial, render: (item) => <div className={styles.primaryCell}><Link href={`/admin/testimonials/${item.id}`} className={styles.cellLink}>{item.testimonial || 'Empty testimonial'}</Link><span className={styles.cellSub}>{item.name}{item.designation ? ` · ${item.designation}` : ''}</span></div> },
  ...
  return <>
    <PageHeader title="Testimonials" description="Published testimonials can be selected for the rotating home page carousel." actions={<AdminButton onClick={() => router.push('/admin/testimonials/new')}><Icon name="plus" size={16} /> New testimonial</AdminButton>} />
    <DataTable table={table} columns={columns} loading={loading} error={error} truncated={truncated} onRetry={refresh} searchPlaceholder="Search testimonials…" empty={<EmptyState title="No testimonials yet" message="Add a testimonial to begin building the home page carousel." action={<AdminButton onClick={() => router.push('/admin/testimonials/new')}>Add testimonial</AdminButton>} />} actions={(item) => <><Link href={`/admin/testimonials/${item.id}`} className={styles.iconAction} title="Edit"><Icon name="edit" size={15} /><span className="visually-hidden">Edit testimonial</span></Link><button type="button" className={styles.iconDanger} onClick={() => setPendingDelete(item)} title="Delete"><Icon name="trash" size={15} /><span className="visually-hidden">Delete testimonial</span></button></>} />
